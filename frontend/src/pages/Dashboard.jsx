...
import { Flame, Beef, Play, ExternalLink, Trash2, Plus, TrendingUp, Utensils, Dumbbell } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const handleAddLog = async (e) => {
    ...
    const dateStr = new Date().toISOString().split('T')[0];
    try {
      await axios.post(`${API_URL}/daily/food`, { ...manualLog, date: dateStr });
  ...
  return (
    <div>
        ...
        <div style={{
          ...
          <form onSubmit={handleAddLog} style={{ display: 'flex', flex: 1, gap: 8 }}>
            <div style={{ flex: 1, position: 'relative' }}>
              ...
              <input type="text" className="chat-input" placeholder="What did you eat? e.g., Grilled chicken..." required value={manualLog.description} onChange={e => setManualLog({...manualLog, description: e.target.value})} style={{ paddingLeft: 38, border: 'none', background: 'transparent', height: 40, boxShadow: 'none' }} />
            </div>
            <div style={{ width: 1, background: 'var(--border-color)', margin: '4px 0' }} />
            <div style={{ width: 100, position: 'relative' }}>
              <input type="number" className="chat-input" placeholder="0" required value={manualLog.calories} onChange={e => setManualLog({...manualLog, calories: e.target.value})} style={{ paddingRight: 40, border: 'none', background: 'transparent', height: 40, boxShadow: 'none' }} />
            ...
            <div style={{ width: 1, background: 'var(--border-color)', margin: '4px 0' }} />
            <div style={{ width: 100, position: 'relative' }}>
              <input type="number" className="chat-input" placeholder="0" required value={manualLog.protein} onChange={e => setManualLog({...manualLog, protein: e.target.value})} style={{ paddingRight: 32, border: 'none', background: 'transparent', height: 40, boxShadow: 'none' }} />
            ...
            <button type="submit" className="primary-btn" style={{ borderRadius: 8, padding: '0 16px', height: 40 }}>
              <Plus size={14} /> Add
            </button>
          </form>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: !plan.isRestDay ? '1fr 1fr' : '1fr', gap: 22, marginBottom: 22 }}>
        {/* Workout Plan */}
        {!plan.isRestDay && (
          <div className="card">
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontWeight: 600, fontSize: 14, color: 'var(--text-primary)', marginBottom: 4 }}>{w.name}</div>
                      <div style={{ fontSize: 13, color: 'var(--text-secondary)' }}>{w.reps}</div>
                    </div>
                    {isSearch ? (
                      <a href={w.video} target="_blank" rel="noreferrer" className="workout-link" style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                        <ExternalLink size={13} /> Watch
                      </a>
                    ) : (
                      <button onClick={() => setVideoUrl(w.video)} className="workout-link" style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                        <Play size={13} /> Watch
                      </button>
                    )}
                  </div>
